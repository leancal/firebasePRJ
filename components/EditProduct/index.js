...
const EditProduct = () => {
  ...
  const handleRemoveFeaturedFeatures = async (featureId) => {
    try {
      if (!selectedProduct || !selectedCollection) {
        console.error('ID de producto o colección no válidos');
        return;
      ...
      // Filtra la característica a eliminar
      const updatedFeaturedFeatures = currentFeaturedFeatures.filter(
        (feature) => feature.id !== featureId
      );

      // Actualiza la base de datos con las características destacadas actualizadas
      await updateDoc(productRef, {
        featuredFeatures: updatedFeaturedFeatures,
      });

      console.log('Característica destacada eliminada con éxito en la base de datos.');

      // Actualiza el estado local para reflejar el cambio inmediatamente en la interfaz
      setFeaturedFeatures(updatedFeaturedFeatures);
    } catch (error) {
      console.error('Error al eliminar la característica destacada de la base de datos:', error);
    }
  };
            ...
            {selectedProduct && (
              <div>
                ...
                <button className="app-content-headerButton" variant="primary" onClick={handleAddFeatureToProduct}>
                  Agregar Característica Destacada
                </button>
                      ...
                      <div style={{ display: 'flex' }}>
                        <li>
                          <button
                            ...
                            className="app-content-headerButton"
                            type="button"
                            onClick={() => handleRemoveFeaturedFeatures(feature.id)}
                          >
                            <CloseIcon fontSize="" />
                          </button>
